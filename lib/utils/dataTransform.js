  ...
  const traverse = (obj, indent = 0, parentNode = null) => {
    ...
    orderKeys.forEach((key) => {
      ...
      if (value.type === "category") {
        ...
        // Add category notes if present
        if (value.notes && value.notes.trim()) {
          // Encode notes to prevent line breaks and pipe characters from breaking format
          const encodedNotes = value.notes.trim().replace(/\|/g, "\\|").replace(/\n/g, "\\n");
          text += ` [NOTES: ${encodedNotes}]`;
        }
        text += "\n";
        if (value.children) {
        ...
        // Format: name | retailSell/bulkSell | cost | sellUnit | costUnit | notes (optional)
...
export const importFromText = (text) => {
  ...
  lines.forEach((line) => {
    ...
    if (content.includes(separator)) {
      ...
      // Parse notes (6th part if present)
      const notes = parts[5] ? parts[5].replace(/\\n/g, "\n").replace(/\\\|/g, "|") : "";
      ...
      // Check if this is a category with notes
      let categoryName = content;
      let categoryNotes = "";
      
      // Pattern: Category Name [NOTES: encoded notes]
      const notesMatch = content.match(/^(.+?)\s*\[NOTES:\s*(.+?)\]$/);
      if (notesMatch) {
        categoryName = notesMatch[1].trim();
        categoryNotes = notesMatch[2].replace(/\\n/g, "\n").replace(/\\\|/g, "|");
      }
      ...
      const newCategory = { 
        type: "category", 
        children: {},
        notes: categoryNotes || "" // Include notes
      };
  ...
  const generateOrderKeys = (obj) => {
    ...
    const keys = Object.keys(container).filter(
      (k) => k !== "type" && k !== "children" && k !== "__orderKeys" && k !== "notes"
    );
  ...
  lines.forEach((line) => {
    ...
    if (content.includes(separator)) {
      ...
      if (parts.length >= 3) {
        ...
        itemsByLevel[indent].notes.push(parts[5] || "");
      }
    }
  });
