...
export const addItem = (priceData, path, type, formData) => {
  ...
  if (type === "category") {
    ...
    target[formData.name] = {
      ...
      bulkSell: parseFloat(formData.bulkSell) || parseFloat(formData.retailSell) || 0,
...
export const editItem = (priceData, parentPath, originalName, newFormData) => {
  ...
  const result = recursiveEditNode(
    ...
    (oldValue) => ({
      ...
      bulkSell: parseFloat(newFormData.bulkSell) || parseFloat(newFormData.retailSell) || 0,
...
const naturalCompare = (a, b) => {
  ...
  String(a).replace(/(\d+\/\d+)|(\d+\.?\d*)|(\D+)/g, (match, frac, num, str) => {
    if (frac) {
  ...
  String(b).replace(/(\d+\/\d+)|(\d+\.?\d*)|(\D+)/g, (match, frac, num, str) => {
    if (frac) {
      // Convert fraction to decimal for comparison
      const decimalValue = fractionToDecimal(frac);
...
export const sortData = (data, sortType) => {
  const getSell = (node) => {
    ...
    if (node.type === "item") {
      ...
      const sellPrice = node.retailSell !== undefined ? node.retailSell : node.sell;
