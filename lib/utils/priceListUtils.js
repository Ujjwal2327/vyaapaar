...
// Check if search string matches across path segments (allows gaps between segments)
const matchesAcrossSegments = (pathArray, searchStr) => {
  if (!searchStr) return true;

  // Try to match the search string by finding it across any combination of segments
  let searchIndex = 0;

  for (let segmentIdx = 0; segmentIdx < pathArray.length; segmentIdx++) {
    const segment = pathArray[segmentIdx];

    for (
      let charIdx = 0;
      charIdx < segment.length && searchIndex < searchStr.length;
      charIdx++
    ) {
      if (segment[charIdx] === searchStr[searchIndex]) {
        searchIndex++;
      }
    }

    // If we've matched the entire search string, return true
...
export const filterData = (data, search) => {
  ...
  const searchInObject = (obj, parentMatches = false, pathArray = []) => {
    const result = {};

    Object.entries(obj).forEach(([key, value]) => {
      const keyNormalized = normalizeItemName(key);
      const currentPathArray = [...pathArray, keyNormalized];

      // Build full hierarchical path by joining all ancestors
      const fullPath = currentPathArray.join("");

      // Check various matching strategies
      const keyMatches = keyNormalized.includes(searchNormalized);
      const fullPathMatches = fullPath.includes(searchNormalized);
      const acrossSegmentsMatch = matchesAcrossSegments(
        currentPathArray,
        searchNormalized
      );

      const shouldInclude =
        parentMatches || keyMatches || fullPathMatches || acrossSegmentsMatch;
      ...
      if (value.type === "category") {
        ...
        if (shouldInclude || hasMatchingChildren) {
          result[key] = {
            ...value,
  ...
  return searchInObject(data, false, []);
};
