...
const matchTokenAgainst = (searchToken, pathToken, allowFuzzy = true) => {
  ...
  if (searchIsDecimal && pathIsFraction) {
    ...
    if (
      pathDecimalVal !== null &&
      Math.abs(searchDecimalVal - pathDecimalVal) < 0.01
    ) {
  ...
  // "exactAndSubstring" mode: only exact and substring, no Levenshtein
  const exactAndSubstringOnly = allowFuzzy === "exactAndSubstring";
  ...
  // Skip Levenshtein distance matching if in exactAndSubstring mode
  if (exactAndSubstringOnly) return 0;

  // LEVENSHTEIN FUZZY MATCHING (only in full fuzzy mode)
...
/**
 * Build a search index from priceData once.
 * Call this whenever priceData changes, then pass the result
 * to filterData to skip the expensive flattenData step on every search.
 */
...
/* Main filterData: Smart search with exact > substring > fuzzy priority
 * Pass a pre-built searchIndex (from buildSearchIndex) to avoid
 * re-flattening and re-tokenizing the entire tree on every keystroke.
 */
export const filterData = (data, search, searchIndex = null) => {
  if (!search) return data;

  const searchTokens = tokenize(search);
  if (searchTokens.length === 0) return {};

  // Use pre-built index if available, otherwise build on the fly (fallback)
  const entries =
    searchIndex && searchIndex.length > 0 ? searchIndex : flattenData(data);

  // PASS 1: Try exact + substring matches (no Levenshtein fuzzy)
  const exactAndSubstringMatches = [];
  ...
  for (const entry of entries) {
    ...
    for (const st of searchTokens) {
      ...
      for (const pt of entry.pathTokens) {
        // Only check exact matches and substring matches (no Levenshtein)
        const score = matchTokenAgainst(st, pt, "exactAndSubstring");
    ...
    if (allMatched) {
      exactAndSubstringMatches.push({ ...entry, score: totalScore });
    }
  }

  // If we found exact or substring matches, return only those
  if (exactAndSubstringMatches.length > 0) {
    exactAndSubstringMatches.sort((a, b) => b.score - a.score);
    return buildResultTreeFromMatches(exactAndSubstringMatches);
  }

  // PASS 2: No exact/substring matches found, now try fuzzy matching (Levenshtein)
  const fuzzyMatches = [];
  ...
  for (const entry of entries) {
    ...
    for (const st of searchTokens) {
      let bestScore = 0;
      for (const pt of entry.pathTokens) {
        const score = matchTokenAgainst(st, pt, true); // Full fuzzy matching
        if (score > bestScore) bestScore = score;
      }
      if (bestScore === 0) {
        allMatched = false;
    ...
    if (allMatched) {
      fuzzyMatches.push({ ...entry, score: totalScore });
    }
  }
  ...
  return buildResultTreeFromMatches(fuzzyMatches);
};
...
export const addItem = (priceData, path, type, formData) => {
  ...
  if (type === "category") {
    ...
    target[formData.name] = {
      type: "category",
      children: {},
      notes: formData.notes || "", // ADD THIS LINE - support category notes
    };
  } else if (type === "item") {
    const target = parent.children || parent;
    target[formData.name] = {
      type: "item",
      retailSell: parseFloat(formData.retailSell) || 0,
      bulkSell:
        parseFloat(formData.bulkSell) || parseFloat(formData.retailSell) || 0,
      cost: parseFloat(formData.cost) || 0,
      sellUnit: formData.sellUnit || "piece",
      costUnit: formData.costUnit || formData.sellUnit || "piece",
      notes: formData.notes || "",
      // Stock tracking is opt-in — when it's off, stockQty/lowStockThreshold
      // are left off the item entirely (not even null) so untouched items
...
export const editItem = (priceData, parentPath, originalName, newFormData) => {
  ...
  const result = recursiveEditNode(
    ...
    (oldValue) => ({
      ...oldValue,
      retailSell: parseFloat(newFormData.retailSell) || 0,
      bulkSell:
        parseFloat(newFormData.bulkSell) ||
        parseFloat(newFormData.retailSell) ||
        0,
      ...
      // Setting these to undefined (rather than omitting the lines) means
      // turning tracking OFF actually clears any previously-saved values —
      // undefined-valued keys are dropped whenever this tree is
      // JSON.stringify'd, which happens on every save and every deep
      // clone, so the fields cleanly disappear rather than lingering.
      stockQty: newFormData.trackStock
        ? parseFloat(newFormData.stockQty) || 0
        : undefined,
      lowStockThreshold: newFormData.trackStock
        ? parseFloat(newFormData.lowStockThreshold) || 0
        : undefined,
    }),
  );
...
export const editCategory = (priceData, categoryPath, newName, newNotes) => {
  const newData = JSON.parse(JSON.stringify(priceData));
  ...
  const result = recursiveEditNode(
    ...
    (oldValue) => ({
      ...oldValue,
      notes: newNotes !== undefined ? newNotes : oldValue.notes,
    }),
  );
...
// Natural sort comparison - handles numbers within strings
// This ensures "500 L" comes before "1000 L" in ascending order
// Also handles fractions: "5/4\"" comes before "2\"" (1.25 < 2)
const naturalCompare = (a, b) => {
  const ax = [];
  const bx = [];

  // Parse string into alternating number/fraction/string segments
  // Match fractions (e.g., 5/4) or regular numbers
  String(a).replace(
    /(\d+\/\d+)|(\d+\.?\d*)|(\D+)/g,
    (match, frac, num, str) => {
      if (frac) {
        // Convert fraction to decimal for comparison
        const decimalValue = fractionToDecimal(frac);
        ax.push([decimalValue !== null ? decimalValue : Infinity, ""]);
      } else if (num) {
        ax.push([parseFloat(num) || Infinity, ""]);
      } else if (str) {
        ax.push([Infinity, str || ""]);
      }
    },
  );
  ...
  String(b).replace(
    ...
    (match, frac, num, str) => {
      if (frac) {
        // Convert fraction to decimal for comparison
        const decimalValue = fractionToDecimal(frac);
        bx.push([decimalValue !== null ? decimalValue : Infinity, ""]);
      } else if (num) {
        bx.push([parseFloat(num) || Infinity, ""]);
      } else if (str) {
        bx.push([Infinity, str || ""]);
      }
    },
  );
...
export const sortData = (data, sortType) => {
  const getSell = (node) => {
    if (!node) return NaN;
    if (node.type === "item") {
      // Use retailSell for sorting, fallback to old 'sell' for backward compatibility
      const sellPrice =
        node.retailSell !== undefined ? node.retailSell : node.sell;
      return Number.isFinite(Number(sellPrice)) ? Number(sellPrice) : NaN;
    }
    return NaN;
  };
        ...
        switch (sortType) {
          case "price-low": {
            ...
            if (aIsCat && bIsCat) {
              ...
              if (aMin === bMin) return naturalCompare(keyA, keyB);
              return aMin - bMin;
            }
          ...
          case "price-high": {
            ...
            if (aIsCat && bIsCat) {
              ...
              if (aMax === bMax) return naturalCompare(keyA, keyB);
              return bMax - aMax;
            }
            return naturalCompare(keyA, keyB);
          }
          ...
          case "alphabetical-reverse":
            return naturalCompare(keyB, keyA);
