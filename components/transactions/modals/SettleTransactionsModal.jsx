...
export const SettleTransactionsModal = ({
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      // Error toast already shown by handleSettle in the page.
    } finally {
      ...
      <DialogContent className="max-w-lg p-0 gap-0 flex flex-col h-[90svh] overflow-hidden">
        <DialogHeader className="px-4 pt-4 pb-3 border-b">
          <div className="flex items-center gap-2">
            <div className="w-9 h-9 rounded-full bg-blue-100 dark:bg-blue-900/60 flex items-center justify-center shrink-0">
        ...
        <ScrollArea className="flex-1 min-h-0 overflow-y-auto">
          <div className="px-4 py-3 space-y-4">
            {noEligible ? (
              ...
              <>
                {/* How it works */}
                <div className="rounded-lg bg-muted/50 px-3 py-3 text-sm text-muted-foreground space-y-1.5">
                ...
                {groups.map(({ txs, dir, isAdvance }) => {
                  ...
                  return (
                    <div key={`${dir}-${isAdvance}`} className="space-y-2">
                      <div className="flex items-center gap-1.5">
                        ...
                        {isAdvance && (
                          <Badge className="text-sm px-2 py-0.5 bg-amber-100 text-amber-700 dark:bg-amber-900/60 dark:text-amber-300 border-0 ml-1">
                            Advance
                          </Badge>
                        )}
                      </div>
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between gap-2 min-w-0">
                                  ...
                                  <p
                                    className={`text-base font-bold shrink-0 tabular-nums whitespace-nowrap ${info.color}`}
                                  >
                                    {fmt(displayAmount)}
                                  </p>
                                </div>
                                <div className="flex items-center gap-2 mt-0.5">
                                  <p className="text-[0.75em] text-muted-foreground font-mono">
                                    ID: {tx.id.slice(0, 16)}…
                                  </p>
                                  {isAdvance && (
                                    <span className="text-[0.75em] text-amber-600 dark:text-amber-400 font-medium">
                                      advance
                                    </span>
                                  )}
                                </div>
                ...
                {hasOnlyOneSide && (
                  <div className="rounded-lg border border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950/30 p-3 flex items-start gap-2 text-sm text-amber-700 dark:text-amber-400">
                    ...
                    <div className="rounded-xl border border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-950/30 p-3 space-y-2">
                      ...
                      {preview.previews.some(
                        ...
                        <div className="flex items-start gap-1.5 rounded-md bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 px-2.5 py-2">
                          <Zap className="w-3.5 h-3.5 shrink-0 mt-0.5 text-amber-600" />
                  ...
                  <>
                    ...
                    <div className="rounded-lg border border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-950/30 p-3 flex items-start gap-2">
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        className="h-8 text-sm shrink-0 text-red-700 dark:text-red-400 hover:text-red-900"
                        onClick={runPreview}
                      >
                        Retry
                      </Button>
